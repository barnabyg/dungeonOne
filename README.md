...
Seed `0` is a short reproducible victory over the goblin in three attacks. Seed `121` reproducibly defeats the fighter in five attacks.
...
3. Enter `inspect ruined archway`, `open wooden door`, and `move guardroom`. Expect the inspected crest, the door to open, the guardroom description, combat to begin, and the fighter's turn.
4. During combat, enter `move reliquary`, `attack`, `status`, and `dance`. Expect each mutation or malformed command to be rejected, status to remain readable, and no attack to occur. Enter `attack goblin` three times. With seed `0`, expect two rounds in which both combatants miss, followed by 8 damage that reduces the goblin from 7 HP to 0 without retaliation.
...
8. Run `npm.cmd start -- --seed 121`, enter the guardroom, and enter `attack goblin` five times. Expect deterministic mechanical output and immediate defeat at 0/20 HP. Expect `look`, `status`, `inventory`, and `help` to remain available, gameplay mutations to be rejected, and `quit` to exit cleanly.
...
Initiative, retreat from active combat, death saves, tactical combat, spells, healing, rests, weight, consumables, equipment switching, AI integration, an external adventure loader, save/resume, in-game restart, deployment, and an installer are intentionally out of scope for issue #6.
